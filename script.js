...
const contagemRegressiva = () => { //functions são elevadas ao topo do escopo na compilação e podem ser chamadas antes de serem declaradas, já usando const + arrow function não são elevadas
    if(tempoDecorridoEmSegundos <= 0) {
        ...
        if (focoAtivo) {
            const evento = new CustomEvent('FocoFinalizado') //usado para disparar o evento em outros arquivos sem uní-los
            document.dispatchEvent(evento) //faz o broadcast do evento
        }
        zerarIntervalo()     
        return
    }
